...
function Table() {
	var columns = [
		{ title: "Lead Name", field: "supplier_name" },
		{ title: "Title", field: "location" },
		{ title: "Organization", field: "email" },
		{ title: "Org Type", field: "phone" },
		{ title: "Email", field: "contact" },
		{ title: "Phone", field: "parts_supplied" },
		{ title: "Last Contact", field: "last_contact", type: "date" },
		{ title: "Notes", field: "info" },
	];
	...
	const handleRowUpdate = (newData, oldData, resolve) => {
		...
		if (newData.supplier_name === "") {
			errorList.push("Please enter supplier_name");
		}
		if (newData.location === "") {
		...
		if (newData.parts_supplied === "") {
			errorList.push("Please enter parts_supplied");
		}
		if (newData.last_contact === "") {
			errorList.push("Please enter last contact");
		}
		if (newData.info === "") {
			errorList.push("Please enter info");
		}

		if (errorList.length < 1) {
			api.patch("/suppliers/" + newData._id, newData)
				.then((res) => {
	...
	const handleRowAdd = (newData, resolve) => {
		...
		if (newData.parts_supplied === undefined) {
			errorList.push("Please enter parts_supplied");
		}
		if (newData.last_contact === undefined) {
			errorList.push("Please enter last contact");
		}
		if (newData.info === undefined) {
			errorList.push("Please enter info");
		}

		if (errorList.length < 1) {
			//no error
			api.post("/suppliers", newData)
				.then((res) => {
					let dataToAdd = [...data];
	...
	const handleRowDelete = (oldData, resolve) => {
		api.delete("/suppliers/" + oldData._id)
			.then((res) => {
