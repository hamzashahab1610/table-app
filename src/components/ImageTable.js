...
function Table({ format }) {
	var columns = [
		...
		{ title: "Image Type", field: "image_type" },
		{
			title: "Option",
			field: "option",
			lookup: { Character: "Character", UI: "UI" },
		},
	];
	...
	const handleRowUpdate = (newData, oldData, resolve) => {
		...
		if (errorList.length < 1) {
			api.patch("/images/" + newData._id, newData)
				.then((res) => {
						...
						actions={[
							{
								...
								onClick: (event, rowData) => {
									...
									window.location.href = rowData.image_url;
								},
							},
							{
								icon: AddBox,
