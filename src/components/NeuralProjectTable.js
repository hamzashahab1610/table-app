...
function Table() {
	...
	const handleRowUpdate = (newData, oldData, resolve) => {
		...
		if (errorList.length < 1) {
			api.patch("/neural_projects/" + newData._id, newData)
				.then((res) => {
	...
	const handleRowAdd = (newData, resolve) => {
		...
		if (errorList.length < 1) {
			//no error
			api.post("/neural_projects", newData)
				.then((res) => {
					let dataToAdd = [...data];
	...
	const handleRowDelete = (oldData, resolve) => {
		api.delete("/neural_projects/" + oldData._id)
			.then((res) => {
